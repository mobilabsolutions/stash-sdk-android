...
object SnackBarExtensions {
    ...
    private val TOP_MARGIN = 75.px
    private val WIDTH = 56.px
    ...
    fun getErrorSnackBar(view: View, errorMessageResourceId: Int): Snackbar {
        val snackBar = Snackbar.make(view, "", Snackbar.LENGTH_LONG)
        val snackBarView = snackBar.view as Snackbar.SnackbarLayout
        snackBarView.setPadding(0, 0, 0, 0)
        val snackView = LayoutInflater.from(view.context).inflate(R.layout.view_error_snackbar, snackBarView, false)
        val close = snackView.findViewById<View>(R.id.close)
        close.setOnClickListener {
            snackBar.view.visibility = View.GONE // We do this to prevent dismissal animation
            snackBar.dismiss()
        }

        val errorTextView = snackView.findViewById<TextView>(R.id.error_text)
        errorTextView.text = view.context.getText(errorMessageResourceId)
        ...
        if (params is CoordinatorLayout.LayoutParams) {
            ...
            with(params as FrameLayout.LayoutParams) {
                ...
                height = WIDTH
                width = FrameLayout.LayoutParams.MATCH_PARENT
                setMargins(16.px, TOP_MARGIN, 16.px, 0)
            }
        }
        snackBar.view.layoutParams = params
        snackBar.view.elevation = 0f
